...
public class ClientInfoDAOImpl extends AbstractDAOImpl implements ClientInfoDAO {
	...
	@Override
	public List<ClientInfo> loadApprovedList() throws ServiceException {
		Connection conn = connectDB();
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			ps = conn.prepareStatement("SELECT acc.*, u.* FROM client_info acc, user u WHERE acc.user_id = u.id and u.status=?");
