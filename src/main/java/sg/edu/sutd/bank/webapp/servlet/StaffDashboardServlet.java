...
public class StaffDashboardServlet extends DefaultServlet {
	...
	private void activateAccount(String[] userEmails, int[] userIds, String[] decisions) throws ServiceException {
		for (int i = 0; i < userIds.length; i++) {
			if (Decision.valueOf(decisions[i]) == Decision.approve) {
				...
				clientAccount.setAccountNumber(AccountNumberGenerator.generate());
	...
	private void onTransactionDecisionAction(HttpServletRequest req, HttpServletResponse resp)
		...
		List<ClientTransaction> approvedTransactions = new ArrayList<ClientTransaction>();
		List<ClientTransaction> declinedTransactions = new ArrayList<ClientTransaction>();
		for (int i = 0; i < transIds.length; i++) {
			int transId = transIds[i];
			Decision decision = Decision.valueOf(decisions[i]);
			if (decision.getStatus() != null && decision.getTransStatus() == TransactionStatus.APPROVED) {
				ClientTransaction trans = new ClientTransaction();
				trans.setId(transId);
				trans.setStatus(decision.getTransStatus());
				approvedTransactions.add(trans );
			} else if(decision.getStatus() != null && decision.getTransStatus() == TransactionStatus.DECLINED) {
				ClientTransaction trans = new ClientTransaction();
				trans.setId(transId);
				trans.setStatus(decision.getTransStatus());
				declinedTransactions.add(trans );
			}
		}
		if (!approvedTransactions.isEmpty()) {
			try {
				clientTransactionDAO.updateApprovedTransaction(approvedTransactions);
			} catch (ServiceException e) {
				sendError(req, e.getMessage());
			}
		}

		if (!declinedTransactions.isEmpty()) {
			try {
				clientTransactionDAO.updatedeclinedTransaction(declinedTransactions);
			} catch (ServiceException e) {
				sendError(req, e.getMessage());
			}
		}
